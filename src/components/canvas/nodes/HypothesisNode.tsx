import React from 'react';
import { Handle, Position } from '@xyflow/react';
import { Lightbulb, CheckCircle2, XCircle, Clock } from 'lucide-react';
import { HypothesisEntity } from '../../../types/research';
import { useResearchStore } from '../../../store/useResearchStore';

interface NodeProps {
  data: {
    entity: HypothesisEntity;
  };
  selected?: boolean;
}

export const HypothesisNode: React.FC<NodeProps> = ({ data, selected }) => {
  const { entity } = data;
  const highlighted = useResearchStore((s) => s.highlightedLineage);
  const isLineageActive =
    highlighted.activeNodeId === entity.id ||
    highlighted.upstreamNodeIds.includes(entity.id) ||
    highlighted.downstreamNodeIds.includes(entity.id);

  const statusIcons = {
    supported: <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />,
    refuted: <XCircle className="h-3.5 w-3.5 text-rose-600" />,
    testing: <Clock className="h-3.5 w-3.5 text-amber-600 animate-spin" />,
    draft: <Clock className="h-3.5 w-3.5 text-slate-400" />,
    abandoned: <XCircle className="h-3.5 w-3.5 text-slate-400" />,
  };

  const statusStyles = {
    supported: 'bg-emerald-50 text-emerald-800 border-emerald-200',
    refuted: 'bg-rose-50 text-rose-800 border-rose-200',
    testing: 'bg-amber-50 text-amber-800 border-amber-200',
    draft: 'bg-slate-50 text-slate-600 border-slate-200',
    abandoned: 'bg-slate-50 text-slate-500 border-slate-200',
  };

  return (
    <div
      id={`node-${entity.id}`}
      className={`group relative w-88 rounded-xl border-2 bg-white/95 p-4 shadow-sm backdrop-blur transition-all duration-150 ${
        selected
          ? 'border-teal-600 ring-4 ring-teal-100 shadow-md scale-[1.02]'
          : isLineageActive
          ? 'border-teal-400 ring-2 ring-teal-50 shadow-sm'
          : 'border-teal-200 hover:border-teal-400 hover:shadow'
      }`}
    >
      <Handle
        type="target"
        position={Position.Left}
        className="!bg-teal-500 !border-white !w-3 !h-3"
      />

      {/* Header */}
      <div className="flex items-center justify-between gap-2 border-b border-teal-50 pb-2.5">
        <div className="flex items-center gap-1.5">
          <div className="flex h-6 w-6 items-center justify-center rounded-md bg-teal-100 text-teal-800">
            <Lightbulb className="h-3.5 w-3.5" />
          </div>
          <span className="font-mono text-xs font-semibold uppercase tracking-wider text-teal-950">
            {entity.code}
          </span>
          <span className="rounded bg-teal-50 px-1.5 py-0.5 text-[10px] font-medium text-teal-700 uppercase">
            Hypothesis
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          {entity.confidence !== undefined && (
            <span className="font-mono text-[11px] font-medium text-teal-700">
              {Math.round(entity.confidence * 100)}%
            </span>
          )}
          <span
            className={`flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium capitalize ${
              statusStyles[entity.status] || statusStyles.draft
            }`}
          >
            {statusIcons[entity.status]}
            {entity.status}
          </span>
        </div>
      </div>

      {/* Body */}
      <div className="mt-2.5">
        <h4 className="line-clamp-2 text-xs font-semibold leading-snug text-slate-900">
          {entity.title || entity.statement.slice(0, 50)}
        </h4>
        <p className="mt-1.5 line-clamp-3 text-[11px] leading-relaxed text-slate-600">
          {entity.statement}
        </p>
      </div>

      <Handle
        type="source"
        position={Position.Right}
        className="!bg-teal-500 !border-white !w-3 !h-3"
      />
    </div>
  );
};
