    ...
    <div className="flex h-full flex-col bg-slate-50 overflow-hidden">
      ...
      <div className="border-b border-slate-200/90 bg-white/90 px-6 py-4 backdrop-blur-md">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-50 border border-indigo-200/80 text-indigo-700 shadow-2xs">
            <Sparkles className="h-4 w-4" />
          </div>
          <div>
            <h2 className="text-base lg:text-lg font-bold text-slate-900 tracking-tight">
              End-to-End Evidence Narrative Lineage
            </h2>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          {stages.map((stage, idx) => (
            <div key={idx} className="relative">
              ...
              {idx < stages.length - 1 && (
                <div className="absolute left-7 top-10 bottom--6 w-0.5 bg-slate-200 -z-10" />
              )}

              <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-2.5">
                    <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100/80 border border-slate-200/60 shadow-2xs">
                      {stage.icon}
                    </div>
                ...
                <div className="mt-3.5 grid grid-cols-1 md:grid-cols-2 gap-3">
                  {stage.items.map((item) => (
                    <div
                      ...
                      className="group cursor-pointer rounded-xl border border-slate-200/80 bg-slate-50/50 p-3.5 hover:border-indigo-400 hover:bg-white transition-all shadow-2xs"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-xs font-bold text-slate-800 bg-white border border-slate-200/60 px-2 py-0.5 rounded-md">
                          {item.code}
                        </span>
                        {item.meta && (
                      ...
                      <p className="mt-2 text-xs font-semibold text-slate-900 line-clamp-2 leading-snug group-hover:text-indigo-600 transition-colors">
                        {item.title}
                      </p>
                    </div>
                  ))}
