...
export const Header: React.FC = () => {
  ...
  return (
    <header className="flex h-14 w-full items-center justify-between border-b border-slate-200 bg-white/95 px-3 lg:px-4 shadow-2xs backdrop-blur select-none z-30 shrink-0">
      ...
      <div className="flex items-center gap-2 lg:gap-3">
        ...
        <div className="relative pl-2 lg:pl-3 border-l border-slate-200" ref={workspaceRef}>
          <button
            onClick={() => setIsWorkspaceMenuOpen(!isWorkspaceMenuOpen)}
            className="flex items-center gap-1.5 rounded-lg bg-slate-100/90 hover:bg-slate-200/80 px-2.5 py-1 text-xs font-semibold text-slate-800 transition cursor-pointer"
          >
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 shrink-0" />
            <span className="max-w-[140px] lg:max-w-[180px] truncate">{currentWorkspaceName}</span>
            ...
            <div className="absolute left-0 top-full mt-1.5 w-64 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl z-50 animate-in fade-in zoom-in-95 duration-100">
              ...
              <div className="border-t border-slate-100 pt-1 space-y-0.5">
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs font-medium text-indigo-600 hover:bg-indigo-50 transition cursor-pointer"
                >
        ...
        {projects.length > 0 && (
          <div className="relative hidden md:block" ref={projectRef}>
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-700 transition cursor-pointer"
            >
        ...
        <span className="font-mono text-[10px] text-slate-400 hidden xl:inline">
      ...
      <nav className="flex items-center gap-1 rounded-xl bg-slate-100/80 p-1 border border-slate-200/60">
        {navItems.map((item) => (
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-2.5 lg:px-3 py-1.5 text-xs font-semibold transition-all ${
              viewMode === item.mode
      ...
      <div className="flex items-center gap-1.5 lg:gap-2">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-2.5 lg:px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors cursor-pointer"
        >
          <Search className="h-3.5 w-3.5 text-slate-400" />
          <span className="hidden xl:inline">Search</span>
          <kbd className="hidden lg:inline rounded bg-white px-1 py-0.2 text-[10px] font-mono text-slate-400 border border-slate-200">
            ⌘K
          </kbd>
        </button>
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-2.5 lg:px-3 py-1.5 text-xs font-semibold text-white shadow-xs hover:from-indigo-700 hover:to-purple-700 transition-all cursor-pointer"
        >
        ...
        <button
          ...
          className="rounded-xl border border-slate-200 bg-white p-2 text-slate-400 hover:bg-slate-50 hover:text-indigo-600 transition-colors cursor-pointer"
        >
          ...
          {isUserMenuOpen && (
            <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-xl z-50 animate-in fade-in zoom-in-95 duration-150">
              ...
              <button
                ...
                className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50 transition cursor-pointer"
              >
                <Users className="h-3.5 w-3.5 text-slate-400" />
                <span>Workspace Members</span>
              </button>
